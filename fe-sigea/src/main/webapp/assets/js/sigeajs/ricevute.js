...
$(document).ready(function() {	
	...
	$('#t_eventifinanziati').on('processing.dt', function(e, settings, processing) {
		if (processing) {
			$('#overlaytable1').show();
	    } else {
	    	$('#overlaytable1').hide();
	...
	var dataTab = $('#t_eventifinanziati').DataTable({
        ...
        columnDefs: [
        	...
        	{ "title": "Creato da", "targets": 1, "width": 250 }, 
        	{ "title": "Titolo", "targets": 2, "width": 197 },
        	{ "title": "Bando", "targets": 3, "width": 197 },  
        	{ "title": "Progetto", "targets": 4, "width": 197 },  
        	{ "title": "Data da", "targets": 5, "width": 70 },
        	{ "title": "Data a", "targets": 6, "width": 70 },
        	{ "title": "Comune", "targets": 7, "width": 200 },
        	{ "title": "ID", "targets": 8, "width": 50 }, 
        	
          	{ targets: 0, "width": 140, searchable: false, orderable: false, visible: true, render: function(data, type, full, meta) {   
				var actionColumn = '';
        		...
        		var btnGestisci = '<button onclick="visualizzaRevisioni(\''+data.eventoId+'\',\''+data.stato+'\',\''+specialcharacters(full.titolo)+'\')" class="btn btn-primary databutton" data-container="body" title="Visualizza revisioni" data-html="true"><i class="dms-wk-icon-concorsi" aria-hidden="true"></i></button>';
            ...
            { targets: 2, "width": 197, searchable: true, orderable: true, visible: true, render: function(data, type, full, meta) {
            	...
            	var terminale = '';
              	if(lugtitolot > 40){      
                	...
                	var data = specialcharacters(full.titolo); // LUIGI full.titolo;
                }              	
             ...
             { targets: 3, "width": 197, searchable: true, orderable: true, visible: true, render: function(data, type, full, meta) {
             	...
             	var terminale = '';
               	if(lugtitolot > 40){      
                 	...
                 	var data = specialcharacters(full.titoloBando); // LUIGI full.titoloBando;
                 }              	
              ...
              { targets: 4, "width": 197, searchable: true, orderable: true, visible: true, render: function(data, type, full, meta) {
  	       		if(full.titoloProgetto != null){
	              	...
	              	var terminale = '';
	                	if(lugtitolot > 40){      
	                  	...
	                  	var data = specialcharacters(full.titoloProgetto); // LUIGI full.titoloProgetto;
	                  }
            	}else{
            		var data =  '';
            	}
        ...
        initComplete: function(settings, json) {
        	...
        	$('#t_eventifinanziati').DataTable().columns.adjust();
           	setTimeout(function(){ 
        		icontabelle(); 
        		$('.dataTables_scrollBody:has(#t_eventifinanziati)').append('<div id="overlaytable1" class="overlaytable" style="display:none;"><div class="spinner"></div></div>'); 
	...
	$('#t_eventifinanziati').on( 'page.dt', function () {
	...
	$('#t_eventifinanziati').on('page.dt', function() { $('.dataTables_scrollBody').scrollTop(0); });
...
$("#cercaevento").click(function(){
	...
	$('#t_eventifinanziati').DataTable().draw();
...
$("#resetevento").click(function(){
	...
	// $("#bandoId")[0].selectize.setValue(""); 
	$('#bandoId').prop('disabled', false);
});

function exportFetch(uuid){
	var xhr = new XMLHttpRequest();  
    xhr.open('GET', context+'/exportEventi/fetch?uuid='+ uuid  +'&ruolo=FINANZIATO_ENTE' , true);
...
$(document).ready(function(){
	...
	function teventisearch() {	
		var dtable = $("#t_eventifinanziati").DataTable();
	
		$("#t_eventifinanziati_filter input")
	...
	function visualizzaRevisioni(idEvento, stato, titolo) {	
	...
	//	var urlGetEvento = context + "/getEvento?idEvento=" + idEvento;		
		var urlGetEvento = context + "/getLogEventi/" + idEvento;		
				...
				$.ajax({// Caricamento json evento
			        ...
			        success: function(response) { 
			        	...
			        	logSet = response;			        	
			        			$("#strutturarevisione").html(titolo);			        	      					
					        },
				...
				var myTable = $('#t_revisionimodal').DataTable({
	        	    ...
	        	    columns: [
	        	        ...
	        	        { name: "data", data: "dataModifica"},//1
	        	        { name: "operazioni", data: "operazioni" },//2
	        	        { name: "utente", data: "nomeUtente" },//3
	        	        { name: "stato", data: "descrizioneStato" }//4
